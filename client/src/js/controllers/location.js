...
/*  Subjectivity Categories */
	...
	function polarity_filter(data, wanted_pol) {
		...
		data.forEach(function(d) {
			switch(wanted_pol) {
				case "negative":
					if (parseFloat(d.sentiment.polarity) < -0.2)
						filtered.push(d)
					break;
				case "neutral":
					if (parseFloat(d.sentiment.polarity) >= -0.2 || d.polarity <= 0.2)
						filtered.push(d)
					break;
				case "positive":
					if (parseFloat(d.sentiment.polarity) > 0.2)
	...
	function subjectivity_filter(data, wanted_sub) {
		...
		data.forEach(function(d) {
			switch(wanted_sub) {
				case "objective":
					if (parseFloat(d.sentiment.subjectivity) < 0.5)
						filtered.push(d);
					break;
				case "subjective":
					if (parseFloat(d.sentiment.subjectivity) >= 0.5)
	...
	uiGmapGoogleMapApi.then(function(maps) {
		...
		function _MapHeatLayer(heatLayer) {
			...
			$scope.general_sentiment.filteredHeatmapData.forEach(function(d) {
				//console.log(d)
				locData.push(new maps.LatLng(parseFloat(d.point.lat), parseFloat(d.point.lon)))
			})
			//console.log(locData);
			heatLayer.setData(new maps.MVCArray(locData))
		};

		function heatmapFilter(data) {
			...
			return polarity_filter(subjectivity_filter($scope.general_sentiment.data.locations, $scope.general_sentiment.filter.polarity), $scope.general_sentiment.filter.subjectivity);
		}

		$scope.general_sentiment.mapUpdateCallback = function() {
			$scope.general_sentiment.filteredHeatmapData = heatmapFilter($scope.general_sentiment.data);
			$scope.general_sentiment.heatmapToggle = true;
			console.log("heatmap reappears")
		}

		$scope.general_sentiment.applyHeatmapFilter = function() {
			$scope.general_sentiment.heatmapToggle = false;
			console.log('heatmap should be gone');
			$scope.general_sentiment.mapUpdateCallback();
		}
