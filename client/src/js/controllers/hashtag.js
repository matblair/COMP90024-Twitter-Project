app.controller("HashtagController", ["$scope", "$http", function($scope, $http) {
	...
	$scope.trending.call = function() {
		$scope.trending.updating = true;
		var parameters = $scope.trending.params;
		parameters.start_date = date_parse($scope.trending.params.start_date);
		parameters.end_date = date_parse($scope.trending.params.end_date);
		$scope.trending.promise = $http.get("http://144.6.227.63:4500/hashtags/trending", {params: parameters}).
	...
	// Calendar Settings
	$scope.trending.toggleMax = function() {
		var date = new Date();
		date.setDate(date.getDate() - 1);
    $scope.maxDate = $scope.maxDate ? null : date;
  };
  $scope.trending.toggleMax();

  $scope.trending.open1 = function($event) {
    $event.preventDefault();
    $event.stopPropagation();

    $scope.trending.date_opened1 = true;
  };

  $scope.trending.open2 = function($event) {
    $event.preventDefault();
    $event.stopPropagation();

    $scope.trending.date_opened2 = true;
  };
